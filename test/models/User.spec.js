...
describe('# User Model', () => {
  ...
  context('associations', () => {
    ...
    before(() => {
      // 將 User model 對 User, Response, Article 做關聯(呼叫 associate)
      User.associate({ Article })
      User.associate({ Response })
      User.associate({ User })
    })

    it('should have many articles', (done) => {
      // 檢查是否有呼叫 hasMany(Article)
      expect(User.hasMany).to.have.been.calledWith(Article)
      done()
    })
    it('should have many responses', (done) => {
      // 檢查是否有呼叫 hasMany(Response)
      expect(User.hasMany).to.have.been.calledWith(Response)
      done()
    })
  ...
  context('action', () => {
    ...
    it('create', (done) => {
      db.User.create({ name: 'AAA', email: 'AAA@', password: '123' })
        .then((user) => {
          data = user
          done()
        })
